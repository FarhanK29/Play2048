...
public class Board {
    ...
    public void addRandomTile() {
        int randomSpotIndex = StdRandom.uniform(0, openSpaces.size());
        double randNum = StdRandom.uniform(0.0,1.0);
        BoardSpot randomSpot = openSpaces.get(randomSpotIndex);
        if(randNum < 0.1)
            gameBoard[randomSpot.getRow()][randomSpot.getCol()] = 4;
        else{
            gameBoard[randomSpot.getRow()][randomSpot.getCol()] = 2;
        }
    }
    ...
    public void swipeLeft() {
        ...
        for(int i = 0; i < gameBoard.length;i++){
            for(int j = 0; j < gameBoard[0].length-1; j++)
            {
                ...
                {
                    if(count == gameBoard[0].length-1)
                    {
                        break;
                    }
                    for(int k = j; k<gameBoard[0].length-1; k++)
                    {
                        gameBoard[i][k] = gameBoard[i][k+1];
                    }
                    gameBoard[i][gameBoard[0].length-1] = 0;
                    count++;
                }
    ...
    public void transpose() {
        for(int i = 0; i < gameBoard.length; i++){
           for(int j = i; j <gameBoard[0].length; j++)
           {
    ...
    public void flipRows() {
        ...
        {
            int temp = gameBoard[row][3];
            gameBoard[row][3] = gameBoard[row][0];
            gameBoard[row][0] = temp;

            temp = gameBoard[row][2];
            gameBoard[row][2] = gameBoard[row][1];
            gameBoard[row][1] = temp;
        }
    }
